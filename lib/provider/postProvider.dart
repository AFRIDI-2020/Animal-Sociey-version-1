  ...
  Future<void> getNotifications() async {
   ...
   final String? myNumber= pref.getString('mobileNo');
    try {
      await FirebaseFirestore.instance
          .collection('Notifications')
          .where('receiverId',isEqualTo: myNumber!).orderBy('date',descending: true)
          .get()
          .then((snapshot) {
        _notificationList.clear();
        _totalNotifications = 0;
        snapshot.docChanges.forEach((element) {
          if (element.doc['postId'] != '' && element.doc['senderId']!= myNumber!) {
      ...
      await FirebaseFirestore.instance
        ...
        value.docChanges.forEach((element) async {
          ...
          await FirebaseFirestore.instance
              ...
              .get()
              .then((snapshot) {
            String name = snapshot['animalName'];
            if (animalName.toLowerCase() == name.toLowerCase()) {
              Post post = Post(
                  postId: snapshot['postId'],
                  postOwnerId: snapshot['postOwnerId'],
                  postOwnerMobileNo: snapshot['postOwnerMobileNo'],
